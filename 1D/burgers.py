...
def run(dx, Tf, generator="numpy", sorder=None, withPlot=True):
    ...
    if withPlot:
        ...
        x = sol1.domain.x
        l1 = ax.plot(x, sol1.m[u], width=1, color='b', label=title1)[0]
        l2 = ax.plot(x, sol2.m[u], width=1, color='g', label=title2)[0]
        le = ax.plot(x, solution(sol1.t, x, xmin, xmax, uL, uR), width=1, color='k', label='exact')[0]

        def update(iframe):
            if sol1.t < Tf:
                sol1.one_time_step()
                sol2.one_time_step()
                l1.set_data(x, sol1.m[u])
                l2.set_data(x, sol2.m[u])
                le.set_data(x, solution(sol1.t, x, xmin, xmax, uL, uR))
